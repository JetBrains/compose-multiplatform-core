...
final class CMPHoverTest: XCTestCase {
    ...
    func testSimulatedHoverReachesHoverRecognizer() {
        ...
        while viewController.recognizer.receivedLocations.isEmpty && Date() < deadline {
            pumpRunLoop(0.05)
        }

        // The view fills the window, so window-space == view-space here.
        XCTAssertEqual(
            viewController.recognizer.receivedLocations, expectedPoints,
            "UIHoverGestureRecognizer received hover events at unexpected points."
        )
    }
}

/// Captures the anchor of every hover event delivered to `shouldReceive(_:)`.
/// Reading the location eagerly (not after the fact) matters because the
/// synthetic dispatch reuses a single `UIEvent` and mutates its anchor
/// between calls.
private final class RecordingHoverRecognizer: UIHoverGestureRecognizer {
    private(set) var receivedLocations: [CGPoint] = []

    override func shouldReceive(_ event: UIEvent) -> Bool {
        if let view = self.view {
            receivedLocations.append(event.cmp_location(in: view))
        }
...
private final class HoverRecordingViewController: UIViewController {
    let recognizer = RecordingHoverRecognizer()
