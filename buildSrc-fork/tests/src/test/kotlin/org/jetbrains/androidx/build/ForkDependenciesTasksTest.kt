...
class ForkDependenciesTasksTest {
    ...
    @Test
    fun `does not change an associated fork artifact version or project, but change type`() {
        val root = createProject(
            original = """
                androidXMultiplatform {
                    sourceSets {
    ...
    @Test
    fun `does not update dependencies when source set verification is suppressed`() {
        val root = createProject(
            original = """
                androidXMultiplatform {
                    sourceSets {
