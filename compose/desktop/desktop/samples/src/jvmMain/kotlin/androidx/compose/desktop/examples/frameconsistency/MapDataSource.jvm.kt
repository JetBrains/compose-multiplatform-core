 ...
 *   ([DataSource.Snapshot.beginTransaction]) stack per thread, nested ones fold silently
...
internal class MapDataSource<K : Any, V> : DataSource {
    ...
    /** The view captured by the legacy [withTransaction] (the flag-off per-pass read pinning). */
    ...
    fun update(vararg entries: Pair<K, V>) {
        ...
        if (transaction != null) {
            ...
            tokens.forEach { DataSource.recordChange(it) }
            // The receiver extension: parked under THIS source's pins only. Every open
            // cycle that shares the source (with the launch-time context: all windows and
            // the application composition) rotates once before an external commit's
            // invalidations are released; unrelated sources are never affected.
            invalidateDependants(tokens)
        }
    }
    ...
    private inner class MapSnapshot : DataSource.Snapshot {
        ...
        override fun endTransaction(frame: Any?, cause: Throwable?) {
            ...
            if (parent != null) {
                ...
                invalidateDependants(transaction.tokens)
            }
        }
