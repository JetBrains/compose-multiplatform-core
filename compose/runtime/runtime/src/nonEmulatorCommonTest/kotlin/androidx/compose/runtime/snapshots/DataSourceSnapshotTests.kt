...
class DataSourceSnapshotTests {
    ...
    fun aNestedIsolateFoldsIntoTheEnclosingBlockAndPublishesNothingItself() {
        // isolate() nests by thread: called inside one of this unit's transactions, the
        ...
        try {
            unit.isolate {
                outerState.value = 1
                unit.isolate { innerState.value = 2 } // nested: folds, no own publication
    ...
    fun explicitIsolationFramesFoldSilentlyAndTheOwningFrameDeliversOnce() {
        ...
        val unit = DataSource.takeSnapshot()
        try {
            val outerFrame = unit.beginIsolation()
            val innerFrame = unit.beginIsolation() // nests inside the outer transaction
            inNested.value = 1
            alsoInOuter.value = 1
            unit.endIsolation(innerFrame, null) // silent fold into the outer transaction
            ...
            unit.endIsolation(outerFrame, null) // publish-through + delivery
