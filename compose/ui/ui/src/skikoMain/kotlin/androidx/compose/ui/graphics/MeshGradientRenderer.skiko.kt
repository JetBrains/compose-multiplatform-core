 ...
 * accelerated through Skia on all Skiko targets.
 */
@Suppress("PrimitiveInCollection")
internal class MeshGradientRendererImpl : MeshGradientRenderer {

    // TODO(CMP-10167 follow-up): extract the shared tessellation math into an abstract base class
    //  that can move to commonMain, with the draw call as the overridable seam. This Skiko class
    //  becomes the default impl (Compose Canvas.drawVertices + BlendMode.Dst); Android extends the
    //  same base with its own optimizations (zero-alloc nativeCanvas.drawVertices + pre-Q buffer).
    ...
    private fun drawPatch(
        ...
        canvas.drawVertices(
            Vertices(
                ...
                // Compose's Vertices requires textureCoordinates to match positions in size, so we
                // reuse the positions list to avoid allocating. We don't want any texturing: the
                // gradient comes entirely from the per-vertex colors.
                textureCoordinates = vertexPositions,
                colors = vertexColors,
                indices = vertexIndices,
            ),
            // Skia's drawVertices blends the vertex colors against the paint's shader using this
            // blend mode. Because textureCoordinates are present (see above), Skia treats the
            // shaderless paint's (opaque black) color as the texture; BlendMode.SrcOver would then
            // paint solid black over the colors. BlendMode.Dst keeps the vertex colors and discards
            // that phantom texture, matching the Android backend which passes texCoords = null.
            BlendMode.Dst,
            paint,
        )
    }
