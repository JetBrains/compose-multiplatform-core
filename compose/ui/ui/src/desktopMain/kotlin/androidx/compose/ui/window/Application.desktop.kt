...
suspend fun awaitApplication(
    ...
    // hosts one application, so that is out of scope. The GlobalSnapshotManager registration below
    // no longer needs this bracket for correctness across app switches — it registers
    // GlobalSnapshotManagerDispatcher, which re-resolves ComposeUIDispatcher per dispatch (see
    // GlobalSnapshotManager.desktop.kt) — but the bracket is still what makes ComposeUIDispatcher
    // resolve to the EDT for this KDT-less path in the first place.
        ...
        withContext(MainUIDispatcher) {
            withContext(YieldFrameClock) {
                val globalSnapshotRegistration =
                    GlobalSnapshotManager.register(GlobalSnapshotManagerDispatcher)
