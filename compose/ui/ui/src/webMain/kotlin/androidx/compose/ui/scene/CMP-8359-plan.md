...
1. **Extract `WebComposeSceneMediator`**, `ComposeWindow` as sole caller — behavior-preserving, verified against existing web tests.
2. **`WebComposeSceneLayer` behind the flag**: own canvas/scene, pointer + keyboard input, focus, scrim, outside-click dismiss, resize/DPI. Popup/Dialog functional end-to-end under the flag.
