...
/**
 * The desktop backends register [GlobalSnapshotManagerDispatcher] with [GlobalSnapshotManager],
 * which captures it into the registration's pump scope. These tests pin the property that makes
 * that safe when a JVM switches
...
class GlobalSnapshotManagerDispatcherTest {
    ...
    fun dispatcherIsStableAcrossReads() {
        // GlobalSnapshotManager keys its shared registrations by dispatcher identity and reads
        // the value once per registration, so it must be the same object every time: the
    ...
    fun captureFollowsTheCurrentOverrideAcrossAnAppSwitch() {
        // Capture the dispatcher ONCE, exactly as GlobalSnapshotManager.register does.
