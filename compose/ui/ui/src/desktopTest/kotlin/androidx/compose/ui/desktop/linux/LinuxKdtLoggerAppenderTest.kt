...
/**
 * [KdtLoggerAppender] adapts this fork's [KLogger]/[BaseLogger] logging shim to KDT's linux
 * `AppenderInterface` so native KDT log output routes through the same logging pipeline as the
 * rest of the JVM side. Pure delegation: every method and enabled-flag forwards to the wrapped
 * [KLogger]. The gtk sibling differs only in the `AppenderInterface` import (the two KDT
 * interfaces are member-identical), so this test covers the delegation shape for both.
 */
@Category(HeadlessTest::class)
class LinuxKdtLoggerAppenderTest {
