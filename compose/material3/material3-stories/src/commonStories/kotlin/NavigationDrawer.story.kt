...
val `ModalNavigationDrawer Story` by story {
    // Parameters for customization
    val initialDrawerState by parameter("Closed")
    val showTopBar by parameter(true)
    val topBarTitle by parameter("Modal Navigation Drawer")
    val drawerTitle by parameter("Navigation Drawer")
    val numItems by parameter(5)

    // State management
    val drawerState = rememberDrawerState(
        initialValue = if (initialDrawerState == "Open") DrawerValue.Open else DrawerValue.Closed
    )
    val scope = rememberCoroutineScope()
    var selectedItem by remember { mutableStateOf(0) }

    // Create items and icons lists
    val items = listOf("Home", "Messages", "Favorites", "Settings", "Profile").take(numItems)
    val icons = listOf(
        Icons.Default.Home,
        Icons.Default.Email,
