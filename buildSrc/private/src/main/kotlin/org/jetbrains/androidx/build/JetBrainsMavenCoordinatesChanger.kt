...
fun Project.changeMavenCoordinatesToJetBrains() {
    ...
    afterEvaluate {
        ...
        check(this.version == version) {
            "The $path version is changed after evaluation from ${this.version} to $version. Check if it is overridden inside build.gradle (for example, via `mavenVersion = `) and remove it"
        }
    }
}
